...
let citation = document.querySelector('#citation.text-center');
let authorName = document.querySelector('#auteur');
let btnNew = document.querySelector('#nouveau');
...
btnNew.addEventListener('click', () => {
  do {
    randomElement = generateInt(citations.length)
  } while (randomElement == lastCitation)

  citation.textContent = citations[randomElement][0];
  authorName.textContent = citations[randomElement][1];
  lastCitation = randomElement;
});
